// To parse this data:
//
//   import { Convert, IGIFList } from "./file";
//
//   const iGIFList = Convert.toIGIFList(json);
//
// These functions will throw an error if the JSON doesn't
// match the expected interface, even if the JSON is valid.
export interface ISmallGif {
  id: string;
  title: string;
  url: string;
}
...
export class Convert {
  public static toIGIFList(json: string): IGIFList {
    return cast(JSON.parse(json), r('IGIFList'));
  }

  public static iGIFListToJson(value: IGIFList): string {
    return JSON.stringify(uncast(value, r('IGIFList')), null, 2);
  }
}
...
const typeMap: any = {
  ...
  Rating: ['g'],
  Type: ['gif'],
  Username: ['detectivepikachumovie', '', 'pokemon'],
};
